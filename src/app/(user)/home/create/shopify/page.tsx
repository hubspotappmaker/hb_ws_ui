...
import { connectShopifyApi } from '@/service/user/source';
...
// Form wrapper for additional mobile styling
...
// Responsive card for mobile adjustments
...
// Space wrapper for mobile button layout
...
const CreateShopify: React.FC = () => {
  ...
  const validateShopUrl = (url: any) => {
    ...
    try
    {
      ...
      // Kiểm tra xem có kết thúc bằng .myshopify.com không
      ...
      // Kiểm tra định dạng hostname hợp lệ
      ...
      // Kiểm tra ký tự hợp lệ trong shop name (chỉ cho phép chữ cái, số, và dấu gạch ngang)
  ...
  const shopUrlValidator = async (_: any, value: any) => {
    ...
    // Tự động cập nhật giá trị với URL đã được sửa (có https://)
  ...
  const handleSubmit = async (values: any) => {
    try
    {
      ...
      // Call the API
      const response = await connectShopifyApi(data);
    ...
    } catch (error)
    {
      ...
      message.error('Failed to connect Shopify store. Please check your credentials and try again.');
    } finally
    {
      setLoading(false);
    }
  ...
  return (
    <PageContainer>
      <HeaderContainer>
        <StyledTitle level={2}>Connect Shopify</StyledTitle>
      </HeaderContainer>
          ...
          >
            ...
            >
              <StyledInput
                ...
                size="large"
                onBlur={(e) => {
                  // Tự động format URL khi người dùng rời khỏi field
            ...
            <ButtonContainer>
              <Space>
                <StyledButton type="default" size="large" onClick={() => window.history.back()}>
                  Cancel
                </StyledButton>
